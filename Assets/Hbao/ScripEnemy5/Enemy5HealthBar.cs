...
public class Enemy5HealthBar : MonoBehaviour
{
    [Header("References")]
    public Enemy5_PhuThuy enemy;
    ...
    private void EnsureEnemyReference()
    {
        if (enemy == null)
        {
            enemy = GetComponentInParent<Enemy5_PhuThuy>();
            if (enemy == null && transform.parent != null)
            {
                enemy = transform.parent.GetComponentInChildren<Enemy5_PhuThuy>();
            }
            if (enemy == null)
            {
                enemy = GetComponent<Enemy5_PhuThuy>();
            }
        }
    }
