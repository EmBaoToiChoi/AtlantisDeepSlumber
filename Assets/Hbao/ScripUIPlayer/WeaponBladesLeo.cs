...
public class WeaponBladesLeo : NetworkBehaviour, IInteractableItem
{
    [Header("Item Settings")]
    public string itemName = "Song kiếm của Leo";
    public float interactRadius = 2.5f;
    public int targetClassIndex = 0; // Sát Thủ (Leo)
    ...
    private void TryCollectWeapon()
    {
        ...
        // Check if player class matches Blades Target (Sát Thủ / index 0)
        if (localPlayer.CharacterClassIndex == targetClassIndex)
        {
            // Success! Unlock weapon 2 and skills, and auto-equip weapon 2
            Debug.Log($"[WeaponBladesLeo] Unlocking dual blades for player {localPlayer.DisplayName}");
        ...
        else
        {
            ...
            Debug.LogWarning($"[WeaponBladesLeo] Player {localPlayer.DisplayName} (class {localPlayer.CharacterClassIndex}) tried to steal Leo's Dual Blades!");
            if (hud != null)
            {
