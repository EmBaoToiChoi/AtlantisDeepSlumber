...
public class LeoPlayer : NetworkBehaviour, IPlayerHUDTarget
{
    ...
    {
        Debug.Log($"[LeoPlayer Debug] PerformRaycastAttack called. isStandaloneMode={isStandaloneMode}, IsOwner={IsOwner}");
        if (!isStandaloneMode && !IsOwner) return;

        Vector3 origin = transform.position + Vector3.up * 1f;
        float range = attackRange;
        Collider[] hits = Physics.OverlapSphere(origin, range);
        Debug.Log($"[LeoPlayer Debug] OverlapSphere found {hits.Length} colliders at origin {origin} with range {range}");
        ...
        foreach (var col in hits)
        {
            ...
            if (IsEnemy(col, out Collider enemyCollider))
            {
                ...
                Debug.Log($"[LeoPlayer Debug] Quét trúng Enemy collider: {col.name}. Khoảng cách: {distance}, Góc: {angle}");
                ...
                if (!alreadyHitEnemies.Contains(enemyRoot))
                {
                    ...
                    {
                        Debug.Log($"[LeoPlayer Debug] Bỏ qua Enemy {enemyRoot.name}: Ngoài góc ({angle} > 75) và khoảng cách ({distance} >= 0.4)");
                    }
                }
            }
            else
            {
                ...
                if (tree != null)
                {
                    ...
                    Debug.Log($"[LeoPlayer Debug] Quét trúng Tree collider: {col.name}. Khoảng cách: {distance}, Góc: {angle}");
                    ...
                    if (!alreadyHitEnemies.Contains(treeRoot))
                    {
                        ...
                        {
                            Debug.Log($"[LeoPlayer Debug] Bỏ qua Tree {tree.name}: Ngoài góc ({angle} > 75) và khoảng cách ({distance} >= 0.4)");
                        }
                    }
                }
            }
