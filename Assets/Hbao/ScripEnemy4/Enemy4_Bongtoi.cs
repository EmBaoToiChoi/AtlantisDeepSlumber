...
public class Enemy4_Bongtoi : NetworkBehaviour
{
    ...
    public Vector3 GetFlankingPosition(Transform target, float desiredDist)
    {
        if (target == null) return transform.position;

        // Shadow Assassin tries to flank behind the player with separation from other assassins
        Vector3 flankOffset = -target.forward * desiredDist + target.right * ((GetHashCode() % 2 == 0 ? 1f : -1f) * 1.5f);
        ...
        Vector3 desiredPos = target.position + flankOffset + separation * 1.2f;
